...
@st.cache(hash_funcs={torch.Tensor: hash_tensor, tokenizers.Tokenizer: lambda x: json.dumps(x.__dict__, sort_keys=True), sqlite3.Connection: lambda x: json.dumps(x.__dict__, sort_keys=True), sqlite3.Cursor: lambda x: hash(x), sqlite3.Row: lambda x: json.dumps(x.__dict__, sort_keys=True)})
